...
16. **M6 — телеметрия + дашборд:** лог метрик в `outputs/telemetry/`, графики
    (`scripts/plot_telemetry.py`), оверлей на `/perception/image` (ID/залоченный/дистанция/FSM/FPS).
    Демо-акцент: совместить M1 (толпа) и M6 (оверлей) в одном заголовочном кадре. Запись демо-видео.
