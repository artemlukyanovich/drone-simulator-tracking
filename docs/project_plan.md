...
[`docs/phase4_setup.md`](phase4_setup.md); реализация не начата). Фокус — не «галочка P→PID», а
устойчивый трекер, приближённый к полю: держать *своего* в толпе, переловить потерянного, честно
мерить дистанцию, вести себя безопасно и **измеримо**. Охват полный (M1–M6):
11. **M1 — мультитрекинг + захват цели:** ByteTrack поверх YOLO (`track(persist=True)`), `track_id`
    в `Target.msg`, lock на одного среди нескольких дистракторов (мир `follow_crowd.sdf`).
