...
13. **M2 — идентичность цели + поиск:** FSM `TAKEOFF→SEARCH→TRACK→LOST` в `follower_node` плюс
    **порт ReID-стека из project_1** (OpenCLIP-эмбеддинги + галерея `ObjectMemory`) в
    `drone_perception`: лок ставится на постоянный `object_id`, а не на временный `track_id`.
    Дрон ведёт **конкретного** человека; не нашёл своего — ищет бессрочно, но никогда не
    подменяет его другим. Actor'ы в `follow_crowd.sdf` делаются различимыми (свой цвет одежды +
    свой рост). Согласовано 2026-07-18, решения Ф4-11…Ф4-15 — `phase4_setup.md` §2.
