...
**Фаза 3 — MVP «вижу → двигаюсь»** ✅ (2026-07-06) (детальный план — `docs/phase3_setup.md`)
...
> ✅ **Гейт Фазы 3 достигнут (2026-07-06).** Пайплайн `detector_node`+`follower_node`
> (`tracking_demo.launch.py`) над **ходячей** целью (Gazebo `<actor>`): дрон детектит идущего
> человека и **физически летит за ним** по сцене. Инкременты 0–4B — `docs/phase3_setup.md`
> §11–§15 (4A: статичная цель, §14; 4B: ходячий actor, §15). Ключевые ловушки реализации:
> `GPU=nvidia` против контеншна рендера (§9); путь цели смещать от точки старта дрона (иначе
> спин, Ф3-6); геометрический предел дистанции у горизонтальной камеры → ретюн forward-канала
> (Ф3-7); `detector_delay_s` против стартовой гонки рендера (Ф3-8). Перцепция реально управляет
> аппаратом — цель проекта (§1) на уровне MVP закрыта.
