...
**Фаза 4 — приближение к реальным условиям** (план согласован 2026-07-07, детали и решения —
[`docs/phase4_setup.md`](phase4_setup.md); **Инкремент 5/M1 — ядро принято 2026-07-15**). Фокус — не
«галочка P→PID», а устойчивый трекер, приближённый к полю: держать *своего* в толпе, переловить
потерянного, честно мерить дистанцию, вести себя безопасно и **измеримо**. Охват полный (M1–M6):
11. ✅ **M1 — мультитрекинг + захват цели:** ByteTrack поверх YOLO (`track(persist=True)`), `track_id`
    в `Target.msg`, lock на одного среди нескольких дистракторов (мир `follow_crowd.sdf`). Ядро
    принято 2026-07-15 (захват+не-перескок); сквозное удержание при полной потере **доставлено
    M3+M2** (наклон камеры убрал дропаут, ReID перезахватывает тот же `object_id`) — гейт закрыт
    полностью. `phase4_setup.md` §8.
12. ✅ **M3 — гимбал/наклон камеры + 3D-слежение + честная дистанция:** камера вниз (снимает
    геометрическую стену 4B), вертикальный канал, `distance_m` по pinhole вместо area-прокси.
    Принят 2026-07-15: наклон снял стену, дистанция в кадре устойчива (~4.6–4.8 м) — `phase4_setup.md` §8.
13. ✅ **M2 — идентичность цели + поиск** (принят 2026-07-18, инкременты 7-0 и 7A):
    FSM `TAKEOFF→TRACK→HOLD→LOST→SEARCH` в `follower_node` плюс
    **порт ReID-стека из project_1** в `drone_perception/reid/` (OpenCLIP-эмбеддинги +
    эталон облика `TargetIdentity` — вместо многообъектного `ObjectMemory`, т.к. цель одна и
    она никогда не протухает): лок стоит на постоянном `object_id`, а не на временном
    `track_id`. Дрон ведёт **конкретного** человека; не нашёл своего — ищет бессрочно, но
    никогда не подменяет его другим. Actor'ы в `follow_crowd.sdf` сделаны различимыми (свой
    цвет одежды + свой рост). Решения Ф4-11…Ф4-17 — `phase4_setup.md` §2, журнал — §8.
    Прямая реализация Р3 (§3): перцепция project_1 переносится как логика в ROS2-ноды.
14. ✅ **M4 — PID** (принят 2026-07-18, инкремент 7B): I+D с anti-windup поверх P на канале
    **дистанции** (дефолт-0 = откат к проверенному P). Диагноз, который его потребовал: чистый P
    за движущейся целью физически не сводит ошибку к нулю — нужна ненулевая ошибка, кормящая
    `kp·e`, отсюда 4.6–4.8 м при уставке 4.0. Интеграл берёт постоянную составляющую скорости на
    себя. Сброс интеграла вне `TRACK` — стык с FSM (M2). PID на канале yaw и feedforward по
    скорости цели **не делались**: доворот претензий не вызывал, настройки под несуществующую
    болезнь не заводим. Детали — `phase4_setup.md` §8.
15. ✅ **M5 — слой безопасности** (принят 2026-07-21, инкремент 8): мягкий клэмп геозоны
    (радиальная скорость наружу гасится) и потолка высоты + RTL по «батарее» (таймер от ARM) и
    по долгому `SEARCH` (выход из бессрочного поиска, Ф4-11). RTL — терминальный, отдан штатному
    режиму PX4 (`VEHICLE_CMD_NAV_RETURN_TO_LAUNCH`), стрим offboard при этом глушится. Остаток
    «батареи» выведен на оверлей камеры (задел M6). Всё в `follower_node`. Детали — `phase4_setup.md` §8.
16. **M6 — живые метрики + проход по нагрузке** (переопределён 2026-07-22, Ф4-18 — детали
    `phase4_setup.md` §2/§8). Изначальный «телеметрия в CSV + графики + демо-видео» пересмотрен:
    **CSV-лог и `plot_telemetry.py` убраны** — всё нужное видно вживую на оверлее и в терминале,
    файлы дали бы лишь офлайн-графики, не востребованные в этом проекте. Демо-видео пользователь
    снимает сам. Разбит на два гейта:
    - ✅ **9A — живые метрики на кадре** (принят 2026-07-22): на `/perception/image` поверх M1–M5
      добавлены **FSM-состояние + FPS + латентность инференса** (`/follower/state` от контроллера,
      FPS/мс меряет детектор). ~30 FPS / ~10 мс. Замер выявил: YOLO лёгкий и **не** узкое место
      (30 FPS упирается в камеру, детектор простаивает ~23 мс/кадр) — это переоценивает 9B.
    - ✅ **9B — проход по нагрузке (в т.ч. GPU)** (закрыт 2026-07-22: **оптимизация не
      требуется**): `nvidia-smi` на живом TRACK показал GPU-Util ~30%, VRAM ~36% (1465/4096 MiB),
      FPS ограничен частотой камеры, а не производительностью GPU — узкого места нет. Средства
      снижения нагрузки (`detect_every_n`, fp16, `imgsz`) зафиксированы на будущее (phase4_setup.md
      §8), сейчас не оправданы. Полученный результат предпочтительнее оптимизации без обоснования:
      инструментирование живых метрик (9A) выполнило свою задачу — потребление измерено, запас
      подтверждён, изменения не вносились.

> ✅ **ФАЗА 4 ЗАВЕРШЕНА (2026-07-22).** Все модули M1–M6 приняты: мультитрекинг+захват (M1),
> наклон камеры + честная дистанция (M3), идентичность+поиск (M2), PID дистанции (M4), слой
> безопасности (M5), живые метрики на кадре (M6/9A) + замер нагрузки без нужды в оптимизации
> (M6/9B). Дрон ведёт конкретного человека в толпе, переловывает потерянного, безопасен
> (геозона/RTL). Остаётся демо-видео (за пользователем). Детали и журнал — `docs/phase4_setup.md`.
