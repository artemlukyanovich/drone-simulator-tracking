...
11. 🟡 **M1 — мультитрекинг + захват цели:** ByteTrack поверх YOLO (`track(persist=True)`), `track_id`
    ...
    принято (захват+не-перескок ✅); удержание при полной потере отложено в M3 (снять геом.стену) +
    M2 (перелов того же id) — `phase4_setup.md` §8.
...
15. **M5 — слой безопасности:** geofence, max altitude, RTL по таймауту потери/«батарее».
