...
16. **M6 — живые метрики + проход по нагрузке** (переопределён 2026-07-22, Ф4-18 — детали
    ...
    **CSV-лог и `plot_telemetry.py` убраны** — всё нужное видно вживую на оверлее и в терминале,
    ...
    - **9B — проход по нагрузке (в т.ч. GPU)** (не начат): снизить нагрузку кодом и конфигом
      (троттлинг детекции, fp16, `imgsz`) с замером «до/после» по метрикам 9A. Честный первый
      шаг — `nvidia-smi` найти реального пожирателя GPU (вероятно рендер Gazebo + резидентный
      ReID), а не оптимизировать заведомо лёгкий детектор вслепую.
