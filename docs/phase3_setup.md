...
| Ф3-4 | **Детектор переносим как логику из project_1**, без трекинга/ReID/counter/CLIP. | Дрону для MVP нужна только детекция одной цели за кадр. Трекинг и (опц.) ReID — Фаза 4 (M1, см. `phase4_setup.md`). Дивергенция от project_1 ожидаема и здорова (решение Р3). |
...
- **P-регулятор** (коэффициенты и лимиты скоростей — в `configs/control/`, без магических
  чисел). Переход к PID — Фаза 4 (M4, см. `phase4_setup.md`).
