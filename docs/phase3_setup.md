...
>
> Статус: 🚧 **в реализации** (2026-06-27). Инкременты 0 ✅ (`drone_interfaces`+`Target.msg`),
...
> Открытый блокер закрыт: причина = контеншн рендера на гибридной графике (lockstep), лечение —
> **`GPU=nvidia`** для Gazebo (§9). Осталось — шаг 4B: ходячий actor в `follow_target.sdf`.
...
| **4B** | Заменить статичную цель на **ходячего** actor'а в `follow_target.sdf`. | при движении цели — следует |
